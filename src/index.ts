...
export default class GrowthBook {
  ...
  private mergeOverrides<T>(experiment: Experiment<T>): Experiment<T> {
    if (this.context.overrides && this.context.overrides[experiment.key]) {
      const { url, ...otherProps } = this.context.overrides[experiment.key];
      experiment = {
        ...experiment,
        ...otherProps,
      };
      if (url) {
        const urlRegex = typeof url === 'string' ? getUrlRegExp(url) : url;
        if (urlRegex) {
          experiment.url = urlRegex;
        }
      }
    }
